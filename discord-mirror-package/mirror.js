...
async function sendToWebhook(webhookUrl, payload, retries = 4) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (result.status === 429 && attempt < retries) {
      ...
      console.warn(`[WARN] Rate limited. Waiting ${retryAfter}s (attempt ${attempt + 1}/${retries})...`);
...
async function sendToWebhookWithFiles(webhookUrl, payload, files, retries = 4) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (result.status === 429 && attempt < retries) {
      let retryAfter = 1;
      try { retryAfter = JSON.parse(result.body).retry_after || 1; } catch (_) {}
      console.warn(`[WARN] Rate limited (files). Waiting ${retryAfter}s (attempt ${attempt + 1}/${retries})...`);
      await sleep(retryAfter * 1000 + 200);
      continue;
    }
...
async function mirror(message) {
  ...
  const content = message.content ? message.content.slice(0, 1990) : undefined;
...
if (!token || token === 'YOUR_TOKEN_HERE' || token === 'PASTE_YOUR_TOKEN_HERE') {
  console.error('[ERROR] No user token set. Open .env and add your token.');
  process.exit(1);
}
